//#region node_modules/@hebcal/core/dist/esm/pkgVersion.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
/** DO NOT EDIT THIS AUTO-GENERATED FILE! */
var version = "6.9.2";
//#endregion
//#region node_modules/@hebcal/hdate/dist/esm/greg.js
...
function yearFromFixed(abs) {
	...
	const n1 = quotient(mod$1(d2, 1461), 365);
...
function greg2abs(date) {
	if (!isDate(date)) throw new TypeError(`not a Date: ${date}`);
	else if (isNaN(date.getTime())) throw new RangeError("Invalid Date");
	return toFixed(date.getFullYear(), date.getMonth() + 1, date.getDate());
}
/**
...
function abs2greg(abs) {
	if (typeof abs !== "number" || isNaN(abs)) throw new TypeError(`not a Number: ${abs}`);
	abs = Math.trunc(abs);
	const year = yearFromFixed(abs);
	const month = quotient(12 * (abs - toFixed(year, 1, 1) + (abs < toFixed(year, 3, 1) ? 0 : isGregLeapYear(year) ? 1 : 2)) + 373, 367);
	const day = abs - toFixed(year, month, 1) + 1;
	const dt = new Date(year, month - 1, day);
	if (year < 100 && year >= 0) dt.setFullYear(year);
	return dt;
}
//#endregion
//#region node_modules/@hebcal/hdate/dist/esm/gregNamespace.js
/*! @hebcal/hdate v0.22.8, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
/**
...
//#endregion
//#region node_modules/@hebcal/hdate/dist/esm/hebrewStripNikkud.js
/*! @hebcal/hdate v0.22.8, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
/**
...
function hebrewStripNikkud(str) {
	if (typeof str !== "string") throw new TypeError(`bad nikkud str: ${str}`);
	return str.normalize().replace(/[\u0590-\u05bd]/g, "").replace(/[\u05bf-\u05c7]/g, "");
}
//#endregion
//#region node_modules/@hebcal/hdate/dist/esm/hdateBase.js
/*! @hebcal/hdate v0.22.8, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var NISAN$5 = 1;
var IYYAR$2 = 2;
var SIVAN$2 = 3;
...
var edCache = /* @__PURE__ */ new Int32Array(2e3);
var EPOCH = -1373428;
...
function assertNumber(n, name) {
	if (typeof n !== "number" || isNaN(n)) throw new TypeError(`param '${name}' not a number: ${n}`);
}
/**
...
function hebrew2abs(year, month, day) {
	...
	if (year < 1) throw new RangeError(`hebrew2abs: invalid year ${year}`);
	let tempabs = day;
	if (month < TISHREI$2) {
		const endMonth = monthsInYear(year);
		for (let m = TISHREI$2; m <= endMonth; m++) tempabs += daysInMonth(m, year);
		for (let m = NISAN$5; m < month; m++) tempabs += daysInMonth(m, year);
	} else for (let m = TISHREI$2; m < month; m++) tempabs += daysInMonth(m, year);
...
function abs2hebrew(abs) {
	...
	abs = Math.trunc(abs);
	if (abs <= EPOCH) throw new RangeError(`abs2hebrew: ${abs} is before epoch`);
	let year = Math.floor((abs - EPOCH) / AVG_HEBYEAR_DAYS);
	while (newYear(year) <= abs) ++year;
	--year;
...
function daysInMonth(month, year) {
	...
	if (d !== 0) return d;
	if (month === ADAR_I$2) return isLeapYear(year) ? 30 : 29;
	if (month === CHESHVAN$2) return longCheshvan(year) ? 30 : 29;
...
function getMonthName(month, year) {
	assertNumber(month, "month");
	assertNumber(year, "year");
	if (month < 1 || month > 14) throw new TypeError(`bad monthNum: ${month}`);
...
function elapsedDays0(year) {
	...
	const mElapsed = 235 * Math.floor(prevYear / 19) + 12 * (prevYear % 19) + Math.floor((prevYear % 19 * 7 + 1) / 19);
	const pElapsed = 204 + 793 * (mElapsed % 1080);
	const hElapsed = 5 + 12 * mElapsed + 793 * Math.floor(mElapsed / 1080) + Math.floor(pElapsed / 1080);
	const parts = pElapsed % 1080 + 1080 * (hElapsed % 24);
	const day = 1 + 29 * mElapsed + Math.floor(hElapsed / 24);
	let altDay = day;
	if (parts >= 19440 || 2 === day % 7 && parts >= 9924 && !isLeapYear(year) || 1 === day % 7 && parts >= 16789 && isLeapYear(prevYear)) altDay++;
	if (altDay % 7 === 0 || altDay % 7 === 3 || altDay % 7 === 5) return altDay + 1;
	else return altDay;
}
/**
...
function monthFromName(monthName) {
	if (typeof monthName === "number") {
		if (isNaN(monthName) || monthName < 1 || monthName > 14) throw new RangeError(`bad monthName: ${monthName}`);
		return monthName;
	}
	if (typeof monthName !== "string") throw new TypeError(`bad monthName: ${monthName}`);
	...
	switch (c[0]) {
		...
		case "נ":
			if (c[1] === "o") break;
			return NISAN$5;
		case "i": return IYYAR$2;
		case "e": return ELUL$1;
		case "c":
		case "ח": return CHESHVAN$2;
		case "k":
		case "כ": return KISLEV$2;
		case "s":
			switch (c[1]) {
				case "i": return SIVAN$2;
				case "h": return SHVAT$1;
			}
			break;
		case "t":
			switch (c[1]) {
				case "a": return TAMUZ$2;
				case "i": return TISHREI$2;
				case "e": return TEVET$2;
			}
			break;
		case "a":
			switch (c[1]) {
				case "v": return AV$2;
				case "d":
					if (/(1|[^i]i|a|א)$/i.test(c)) return ADAR_I$2;
					return ADAR_II$2;
			}
			break;
		case "ס": return SIVAN$2;
		case "ט": return TEVET$2;
		case "ש": return SHVAT$1;
		case "א":
			switch (c[1]) {
				case "ב": return AV$2;
				case "ד":
					if (/(1|[^i]i|a|א)$/i.test(c)) return ADAR_I$2;
					return ADAR_II$2;
				case "י": return IYYAR$2;
				case "ל": return ELUL$1;
			}
			break;
		case "ת": switch (c[1]) {
			case "מ": return TAMUZ$2;
			case "ש": return TISHREI$2;
		}
	}
	throw new RangeError(`bad monthName: ${monthName}`);
}
//#endregion
//#region node_modules/@hebcal/hdate/dist/esm/anniversary.js
/*! @hebcal/hdate v0.22.8, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var NISAN$4 = months.NISAN;
...
function toSimpleHebrewDate(obj) {
	...
	} else if (isDate(obj)) return abs2hebrew(greg2abs(obj));
	else return abs2hebrew(obj);
}
/**
...
function getYahrzeitHD(hyear, date) {
	...
	if (hyear <= hDeath.yy) return;
	if (hDeath.mm === CHESHVAN$1 && hDeath.dd === 30 && !longCheshvan(hDeath.yy + 1)) hDeath = abs2hebrew(hebrew2abs(hyear, KISLEV$1, 1) - 1);
	else if (hDeath.mm === KISLEV$1 && hDeath.dd === 30 && shortKislev(hDeath.yy + 1)) hDeath = abs2hebrew(hebrew2abs(hyear, TEVET$1, 1) - 1);
	else if (hDeath.mm === ADAR_II$1) hDeath.mm = monthsInYear(hyear);
...
function getBirthdayHD(hyear, date) {
	...
	if (hyear === origYear) return orig;
	else if (hyear < origYear) return;
	const isOrigLeap = isLeapYear(origYear);
	let month = orig.mm;
	let day = orig.dd;
	if (month === ADAR_I$1 && !isOrigLeap || month === ADAR_II$1 && isOrigLeap) month = monthsInYear(hyear);
...
//#endregion
//#region node_modules/@hebcal/hdate/dist/esm/gematriya.js
/*! @hebcal/hdate v0.22.8, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var GERESH = "׳";
var GERSHAYIM = "״";
var heb2num = {
...
var num2heb = {};
for (const [key, val] of Object.entries(heb2num)) num2heb[val] = key;
function num2digits(num) {
	...
	while (num > 0) {
		...
		let incr = 100;
		let i;
		for (i = 400; i > num; i -= incr) if (i === incr) incr = incr / 10;
		digits.push(i);
		num -= i;
	}
	return digits;
...
function gematriya(num) {
	...
	if (!num1 || num1 < 0) throw new TypeError(`invalid number: ${num}`);
	let str = "";
	const thousands = Math.floor(num1 / 1e3);
	if (thousands > 0 && thousands !== 5) {
		const tdigits = num2digits(thousands);
		for (const tdig of tdigits) str += num2heb[tdig];
		str += GERESH;
	}
...
function gematriyaStrToNum(str) {
	if (typeof str !== "string") throw new TypeError(`bad gematriya str: ${str}`);
	let num = 0;
...
//#endregion
//#region node_modules/@hebcal/hdate/dist/esm/pad.js
/*! @hebcal/hdate v0.22.8, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
/**
...
//#endregion
//#region node_modules/@hebcal/hdate/dist/esm/dateFormat.js
/*! @hebcal/hdate v0.22.8, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
...
function getPseudoISO(tzid, date) {
	...
	if (m === null) throw new Error(`Unable to parse formatted string: ${str}`);
	let hour = m[4];
	if (hour === "24") hour = "00";
...
//#endregion
//#region node_modules/@hebcal/hdate/dist/esm/ashkenazi.po.js
/*! @hebcal/hdate v0.22.8, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var ashkenazi_po_default = {
	"headers": {
		"plural-forms": "nplurals=2; plural=(n > 1);",
		"language": "und-x-ashkenaz"
	},
	"contexts": { "": { "Tevet": ["Teves"] } }
};
//#endregion
//#region node_modules/@hebcal/hdate/dist/esm/he.po.js
/*! @hebcal/hdate v0.22.8, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var he_po_default = {
	"headers": {
		"plural-forms": "nplurals=2; plural=(n > 1);",
...
//#endregion
//#region node_modules/@hebcal/hdate/dist/esm/locale.js
/*! @hebcal/hdate v0.22.8, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var noopLocale = {
...
function checkLocale(locale) {
	if (typeof locale !== "string") throw new TypeError(`Invalid locale name: ${locale}`);
...
function getExistingLocale(locale) {
	...
	if (!loc) throw new RangeError(`Locale '${locale}' not found`);
	return loc;
}
/**
...
var Locale = class Locale {
	...
	*/
	static lookupTranslation(id, locale) {
		const array = (typeof locale === "string" && locales.get(checkLocale(locale)) || noopLocale.contexts[""])[id];
		if (array?.length && array[0].length) return array[0];
	}
	/**
	...
	*/
	static addLocale(locale, data) {
		...
		if (typeof ctx !== "object" || typeof ctx[""] !== "object") throw new TypeError(`Locale '${locale}' invalid compact format`);
	...
	*/
	static addTranslation(locale, id, translation) {
		...
		if (typeof id !== "string" || id.length === 0) throw new TypeError(`Invalid id string: ${id}`);
		const isArray = Array.isArray(translation);
		if (isArray) {
			const t0 = translation[0];
			if (typeof t0 !== "string" || t0.length === 0) throw new TypeError(`Invalid translation array: ${translation}`);
		} else if (typeof translation !== "string") throw new TypeError(`Invalid translation string: ${translation}`);
	...
	*/
	static getLocaleNames() {
		return Array.from(locales.keys()).sort((a, b) => a.localeCompare(b));
	}
	/**
	...
	*/
	static ordinal(n, locale) {
		const locale1 = checkLocale(locale || "");
		if (locale1 === "en" || locale1.startsWith("ashkenazi")) return getEnOrdinal(n);
		else if (Locale.isHebrewLocale(locale1)) return String(n);
		else if (locale1 === "es") return n + "º";
		return n + ".";
	}
	/**
	...
	*/
	static copyLocaleNoNikud(data) {
		...
		const m = {};
		for (const [key, val] of Object.entries(strs)) m[key] = [hebrewStripNikkud(val[0])];
		return {
...
var poHeNoNikud$1 = Locale.copyLocaleNoNikud(he_po_default);
Locale.addLocale("he-x-NoNikud", poHeNoNikud$1);
//#endregion
//#region node_modules/@hebcal/hdate/dist/esm/hdate.js
/*! @hebcal/hdate v0.22.8, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
function mod(x, y) {
	return x - y * Math.floor(x / y);
}
...
var HDate = class HDate {
	...
	*/
	constructor(day, month, year) {
		if (arguments.length === 2 || arguments.length > 3) throw new TypeError("HDate constructor requires 0, 1 or 3 arguments");
		if (arguments.length === 3) {
			this.dd = this.mm = 1;
			const yy = typeof year === "string" ? parseInt(year, 10) : year;
			if (isNaN(yy)) throw new TypeError(`HDate called with bad year: ${year}`);
			this.yy = yy;
			setMonth(this, month);
			const dd = typeof day === "string" ? parseInt(day, 10) : day;
			if (isNaN(dd)) throw new TypeError(`HDate called with bad day: ${day}`);
			setDate(this, dd);
		} else {
			if (day === void 0 || day === null) day = /* @__PURE__ */ new Date();
			const abs0 = typeof day === "number" && !isNaN(day) ? day : isDate(day) ? greg2abs(day) : isSimpleHebrewDate(day) ? day : null;
			if (abs0 === null) throw new TypeError(`HDate called with bad arg: ${day}`);
			const isNumber = typeof abs0 === "number";
			const d = isNumber ? abs2hebrew(abs0) : abs0;
			this.yy = d.yy;
			this.mm = d.mm;
			this.dd = d.dd;
			if (isNumber) this.rd = abs0;
		}
	}
	/**
	...
	*/
	abs() {
		if (typeof this.rd !== "number") this.rd = hebrew2abs(this.yy, this.mm, this.dd);
		return this.rd;
	}
	/**
	* Converts Hebrew date to R.D. (Rata Die) fixed days.
	...
	*/
	render(locale, showYear = true) {
		...
		const monthName = Locale.gettext(this.getMonthName(), locale0).replace(/'/g, "’");
		const dateStr = `${Locale.ordinal(day, locale0)}${getDayOfTranslation(locale0)} ${monthName}`;
		if (showYear) return `${dateStr}, ${this.getFullYear()}`;
		else return dateStr;
	}
	/**
	...
	*/
	add(amount, units = "d") {
		...
		if (!amount) return new HDate(this);
		units = standardizeUnits(units);
		if (units === UNITS_DAY) return new HDate(this.abs() + amount);
		else if (units === UNITS_WEEK) return new HDate(this.abs() + 7 * amount);
		else if (units === UNITS_YEAR) return new HDate(this.getDate(), this.getMonth(), this.getFullYear() + amount);
		else {
			let hd = new HDate(this);
			const sign = amount > 0 ? 1 : -1;
			amount = Math.abs(amount);
			for (let i = 0; i < amount; i++) hd = new HDate(hd.abs() + sign * hd.daysInMonth());
			return hd;
		}
	}
	/**
	...
	*/
	toString() {
		...
		return `${day} ${this.getMonthName()} ${fullYear}`;
	}
	/**
	* Returns true if Hebrew year is a leap year
	* @param year Hebrew year
	...
	*/
	static monthNum(month) {
		if (typeof month === "number") {
			if (isNaN(month) || month > 14) throw new RangeError(`bad monthNum: ${month}`);
			return month;
		}
		if (typeof month !== "string") throw new TypeError(`bad monthNum: ${month}`);
	...
	*/
	static fromGematriyaString(str, currentThousands = 5e3) {
		...
		if (numParts !== 3 && numParts !== 4) throw new RangeError(`cannot parse gematriya str: "${str}"`);
		const day = gematriyaStrToNum(parts[0]);
		const month = monthFromName(numParts === 3 ? parts[1] : parts[1] + " " + parts[2]);
		let year = gematriyaStrToNum(numParts === 3 ? parts[2] : parts[3]);
		if (year < 1e3) year += currentThousands;
...
function fixDate(hd) {
	...
	if (hd.dd > daysInMonth(hd.mm, hd.yy)) {
		...
		hd.dd -= daysInMonth(hd.mm, hd.yy);
		if (hd.mm === monthsInYear(hd.yy)) hd.mm = 1;
		else hd.mm += 1;
		fix(hd);
	}
	fixMonth(hd);
...
//#endregion
//#region node_modules/@hebcal/hdate/dist/esm/anniversaryHDate.js
/*! @hebcal/hdate v0.22.8, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
/**
...
//#endregion
//#region node_modules/@hebcal/hdate/dist/esm/index.js
/*! @hebcal/hdate v0.22.8, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
//#endregion
//#region node_modules/@hebcal/core/dist/esm/ashkenazi.po.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var poAshkenazi = {
	"headers": {
		"plural-forms": "nplurals=2; plural=(n > 1);",
	...
	"contexts": { "": {
		...
		"Rosh Hashana LaBehemot": ["Rosh Hashana LaBeheimos"],
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/he.po.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var poHe = {
	"headers": {
		"plural-forms": "nplurals=2; plural=(n > 1);",
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/he-x-NoNikud.po.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var noNikudOverride = {
	"headers": {
		"plural-forms": "nplurals=2; plural=(n != 1);",
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/locale.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/event.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
/**
...
var Event = class {
	...
	*/
	constructor(date, desc, mask = 0, attrs) {
		if (!HDate.isHDate(date)) throw new TypeError(`Invalid Event date: ${date}`);
		if (typeof desc !== "string") throw new TypeError(`Invalid Event description: ${desc}`);
		this.date = date;
		this.desc = desc;
		this.mask = +mask;
		if (typeof attrs === "object" && attrs !== null) Object.assign(this, attrs);
	}
	/**
	* Hebrew date of this event
	*/
	...
	*/
	getCategories() {
		...
		for (const attrs of flagToCategory) if (mask & attrs[0]) return attrs.slice(1);
		return ["unknown"];
	}
};
//#endregion
//#region node_modules/@hebcal/core/dist/esm/HebrewDateEvent.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
/** Daily Hebrew date ("11th of Sivan, 5780") */
var HebrewDateEvent = class extends Event {
	...
	*/
	render(locale) {
		const locale0 = locale?.toLowerCase() ?? "en";
		const hd = this.getDate();
		switch (locale0) {
			case "h":
			case "he": return hd.renderGematriya(false);
			case "he-x-nonikud": return hd.renderGematriya(true);
			default: return hd.render(locale0, true);
		}
	}
	/**
	* @private
	...
	*/
	renderBrief(locale) {
		const locale0 = locale?.toLowerCase() ?? "en";
		const hd = this.getDate();
		if (hd.getMonth() === months.TISHREI && hd.getDate() === 1) return this.render(locale0);
		switch (locale0) {
			case "h":
			case "he":
			case "he-x-nonikud": return this.renderBriefHebrew(locale0);
			default: return hd.render(locale0, false);
		}
	}
};
//#endregion
...
function createDateTimeFormatShell(createArgsProvider, transformOptions = identity) {
	...
	class ShimDateTimeFormat {
		constructor(locales, options = Object.create(null)) {
			...
			const baseFormat = new RawDateTimeFormat(locales, new Proxy(Object.create(null), { get(_target, name) {
		...
		const getTemporalFormat = memoize((branding) => {
			...
			switch (branding) {
				...
				case PlainMonthDayBranding:
					...
					break;
				default: throwTypeError(invalidFormatType(branding));
			}
			return new RawDateTimeFormat(internals.ze, options);
		});
		return {
			A(formattable) {
				if (void 0 === formattable) return [internals.t];
				const brandingAndSlots = getTemporalBrandingAndSlots(formattable);
				if (!brandingAndSlots) return [internals.t, Number(formattable)];
			...
			v(start, end) {
				...
				if (!startBrandingAndSlots && !endBrandingAndSlots) return [
					internals.t,
					startEpochMilli,
					endEpochMilli
				];
...
function checkTemporalDateTimeFormatCompatible(format, branding, slots) {
	switch (branding) {
		...
		case PlainYearMonthBranding:
		case PlainMonthDayBranding:
			checkResolvedCalendarCompatible(format, slots, 1);
			return;
		default: throwTypeError(invalidFormatType(branding));
	}
}
function temporalDateTimeToEpochMilli(branding, slots) {
	switch (branding) {
		case InstantBranding: return getEpochMilli(slots);
		case PlainDateTimeBranding: return isoDateTimeToEpochMilli(slots);
		case PlainDateBranding:
		case PlainYearMonthBranding:
		case PlainMonthDayBranding: return isoDateToEpochMilli(slots);
		case PlainTimeBranding: return timeFieldsToMilli(slots);
		default: throwTypeError(invalidFormatType(branding));
	}
}
//#endregion
...
var { toTemporalInstant } = { toTemporalInstant() {
...
//#endregion
//#region node_modules/temporal-polyfill/shim.js
function installImplementation() {
	Object.defineProperties(globalThis, createPropDescriptors({ Temporal: Temporal$1 })), Object.defineProperties(Intl, createPropDescriptors({ DateTimeFormat })), Object.defineProperties(Date.prototype, createPropDescriptors({ toTemporalInstant }));
}
function install() {
...
install();
//#endregion
//#region node_modules/@hebcal/noaa/dist/index.js
/**
...
var GeoLocation = class {
	...
	*/
	setElevation(elevation) {
		...
		if (elevation < 0) throw new RangeError(`elevation ${elevation} must be zero or positive`);
		this.elevation = elevation;
	}
	setLatitude(latitude) {
		if (typeof latitude !== "number") throw new TypeError("Invalid latitude");
		if (latitude < -90 || latitude > 90) throw new RangeError(`Latitude ${latitude} out of range [-90,90]`);
	...
	*/
	...
	setLongitude(longitude) {
		...
		if (longitude < -180 || longitude > 180) throw new RangeError(`Longitude ${longitude} out of range [-180,180]`);
	...
	*/
	setTimeZone(timeZoneId) {
		if (!timeZoneId) throw new RangeError("Invalid timeZoneId");
...
*/
var NOAACalculator = class NOAACalculator {
	/**
	...
	*/
	static getTimeOffset(time, offset) {
		if (time === null || isNaN(offset)) return null;
	...
	*/
	getElevationAdjustment(elevation) {
		return radiansToDegrees(Math.acos(earthRadius / (earthRadius + elevation / 1e3)));
	}
	/**
	...
	*/
	adjustZenith(zenith, elevation) {
		...
		if (zenith === NOAACalculator.GEOMETRIC_ZENITH) adjustedZenith = zenith + (.8333333333333333 + this.getElevationAdjustment(elevation));
	...
	*/
	getTemporalHour(startOfDay = this.getSeaLevelSunrise(), endOfDay = this.getSeaLevelSunset()) {
		if (startOfDay === null || endOfDay === null) return NaN;
	...
	*/
	getDateFromTime(time, isSunrise) {
		...
		if (isNaN(epochMillis)) return null;
	...
	*/
	getEpochMillisFromTime(time, isSunrise) {
		...
		let dayOffset = 0;
		if (isSunrise && localTimeHours + hours > 18) dayOffset = -1;
		else if (!isSunrise && localTimeHours + hours < 6) dayOffset = 1;
		const year = cal.year;
	...
	*/
	static getEarthOrbitEccentricity(julianCenturies) {
		return .016708634 - julianCenturies * (42037e-9 + 1.267e-7 * julianCenturies);
	}
	/**
	...
	*/
	static getSunEquationOfCenter(julianCenturies) {
		const mrad = degreesToRadians(NOAACalculator.getSunGeometricMeanAnomaly(julianCenturies));
		const sinm = Math.sin(mrad);
		const sin2m = Math.sin(mrad + mrad);
		const sin3m = Math.sin(mrad + mrad + mrad);
		return sinm * (1.914602 - julianCenturies * (.004817 + 14e-6 * julianCenturies)) + sin2m * (.019993 - 101e-6 * julianCenturies) + sin3m * 289e-6;
	}
	/**
	...
	*/
	static getSunTrueLongitude(julianCenturies) {
		return NOAACalculator.getSunGeometricMeanLongitude(julianCenturies) + NOAACalculator.getSunEquationOfCenter(julianCenturies);
	}
	/**
	...
	*/
	static getSunApparentLongitude(julianCenturies) {
		...
		return sunTrueLongitude - .00569 - .00478 * Math.sin(degreesToRadians(omega));
	}
	/**
	...
	*/
	static getMeanObliquityOfEcliptic(julianCenturies) {
		return 23 + (26 + (21.448 - julianCenturies * (46.815 + julianCenturies * (59e-5 - julianCenturies * .001813))) / 60) / 60;
	}
	/**
	...
	*/
	static getObliquityCorrection(julianCenturies) {
		...
		const omega = 125.04 - 1934.136 * julianCenturies;
		return obliquityOfEcliptic + .00256 * Math.cos(degreesToRadians(omega));
	}
	/**
	...
	*/
	static getSunDeclination(julianCenturies) {
		...
		return radiansToDegrees(Math.asin(sint));
	}
	/**
	...
	*/
	static getEquationOfTime(julianCenturies) {
		...
		return radiansToDegrees(y * sin2l0 - 2 * eccentricityEarthOrbit * sinm + 4 * eccentricityEarthOrbit * y * sinm * cos2l0 - .5 * y * y * sin4l0 - 1.25 * eccentricityEarthOrbit * eccentricityEarthOrbit * sin2m) * 4;
	}
	/**
	...
	*/
	static getSolarElevation(date, lat, lon) {
		...
		const decRad = degreesToRadians(NOAACalculator.getSunDeclination(julianCenturies));
		const latRad = degreesToRadians(lat);
	...
	*/
	static getSolarNoonUTC(julianCenturies, longitude) {
		...
		const newt = NOAACalculator.getJulianCenturiesFromJulianDay(NOAACalculator.getJulianDayFromJulianCenturies(julianCenturies) - .5 + solNoonUTC / 1440);
		eqTime = NOAACalculator.getEquationOfTime(newt);
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/cities.json.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var citiesJson = [
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/location.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
...
/** @private */
var timeFormatCache = new QuickLRU({ maxSize: 120 });
/**
...
var Location = class extends GeoLocation {
	...
	*/
	constructor(latitude, longitude, il, tzid, cityName, countryCode, geoid, elevation) {
		...
		if (isNaN(lat) || lat < -90 || lat > 90) throw new RangeError(`Latitude ${latitude} out of range [-90,90]`);
		const long = typeof longitude === "number" ? longitude : parseFloat(longitude);
		if (isNaN(long) || long < -180 || long > 180) throw new RangeError(`Longitude ${longitude} out of range [-180,180]`);
		if (!tzid) throw new RangeError("Invalid timezone");
		const elev = typeof elevation === "number" && elevation > 0 ? elevation : 0;
		if (cityName && typeof cityName !== "string") cityName = String(cityName);
		super(cityName || null, lat, long, elev, tzid);
		this.il = Boolean(il);
		this.cc = countryCode;
		if (countryCode === "IL") this.il = true;
		this.geoid = geoid;
	}
	/**
	...
	*/
	getShortName() {
		...
		if (this.cc === "US" && name[comma + 2] === "D") {
			if (name[comma + 3] === "C") return name.substring(0, comma + 4);
			if (name[comma + 3] === "." && name[comma + 4] === "C") return name.substring(0, comma + 6);
		}
	...
	*/
	static lookup(name) {
		if (classicCities.size === 0) initClassicCities();
	...
	*/
	static legacyTzToTzid(tz, dst) {
		...
		if (dst === "usa") return ZIPCODES_TZ_MAP[String(tz * -1)];
	}
	/**
	...
	*/
	static getUsaTzid(state, tz, dst) {
		...
		if (tz === 7 && state === "AZ") return dst === "Y" ? "America/Denver" : "America/Phoenix";
	...
	*/
	static addLocation(cityName, location) {
		...
		if (classicCities.has(name)) return false;
		classicCities.set(name, location);
		return true;
	}
};
//#endregion
//#region node_modules/@hebcal/core/dist/esm/reformatTimeStr.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var hour12cc = {
	US: 1,
	CA: 1,
...
function reformatTimeStr(timeStr, suffix, options) {
	...
	if (hour12 !== void 0 && !hour12) return timeStr;
	if (!hour12 && hour12cc[cc] === void 0) return timeStr;
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/moladBase.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
/**
...
function getChalakimSinceMoladTohu(year, month) {
	...
	return CHALAKIM_MOLAD_TOHU + CHALAKIM_PER_MONTH * (235 * Math.trunc((year - 1) / 19) + 12 * ((year - 1) % 19) + Math.trunc((7 * ((year - 1) % 19) + 1) / 19) + (monthOfYear - 1));
}
/**
...
function calculateMolad(year, month) {
	...
	let hd = new HDate(moladToAbsDate(chalakim));
	const conjunctionDay = Math.trunc(chalakim / CHALAKIM_PER_DAY);
	let adjustedChalakim = Math.trunc(chalakim - conjunctionDay * CHALAKIM_PER_DAY);
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/moladDate.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
/** constant for milliseconds in a minute (60,000) */
var MINUTE_MILLIS = 6e4;
/**
...
function getMoladAsDate(molad) {
	...
	const offset = getLocalMeanTimeOffset(dt, 35.2354, tzid);
	return zdt.subtract({ milliseconds: offset }).withTimeZone("UTC");
}
//#endregion
//#region node_modules/@hebcal/core/dist/esm/string.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/molad.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var enDoW = [
	"Sunday",
	"Monday",
...
function getDayNames(locale) {
	if (Locale.isHebrewLocale(locale)) return heDayNames;
	if (locale === "fr") return frDoW;
	return enDoW;
}
...
var Molad = class {
	...
	*/
	getTchilasZmanKidushLevana3Days() {
		return this.getInstant().add({ hours: 72 });
	}
	/**
	...
	*/
	getTchilasZmanKidushLevana7Days() {
		return this.getInstant().add({ hours: 168 });
	}
	/**
	...
	*/
	getSofZmanKidushLevana15Days() {
		return this.getInstant().add({ hours: 360 });
	}
	/**
	...
	*/
	render(locale, options) {
		...
		const dow = getDayNames(locale)[this.getDow()];
		...
		if (Locale.isHebrewLocale(locale)) {
			let result = `${moladStr} ${monthName} יִהְיֶה בַּיּוֹם ${dow} בשָׁבוּעַ, בְּשָׁעָה ${hour} ${getHebrewTimeOfDay(hour)}, ו-${minutes} ${minutesStr}`;
			if (chalakim !== 0) result += ` ו-${chalakim} ${chalakimStr}`;
			if (locale.toLocaleLowerCase() === "he-x-nonikud") return Locale.hebrewStripNikkud(result);
			return result;
		}
		const fmtTime = reformatTimeStr(`${hour}:${pad2(minutes)}`, "pm", options);
		const result = `${moladStr} ${smartApostrophe(monthName)}: ${dow}, ${fmtTime}`;
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/zmanim.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
/**
...
var Zmanim = class Zmanim {
	...
	*/
	gregEve() {
		...
		return new Zmanim(this.gloc, prev, this.useElevation).sunset();
	}
	/**
	...
	*/
	chatzot() {
		...
		return millisToDate(startOfDay + temporalHourMillis(startOfDay, this.sunsetMillis(GEOMETRIC_ZENITH, false)) * 6);
	}
	/**
	...
	*/
	chatzotNight() {
		return /* @__PURE__ */ new Date(this.sunrise().getTime() - this.nightHour() * 6);
	}
	/**
	...
	*/
	...
	getShaahZmanisBasedZmanMillis(startOfDay, endOfDay, hours) {
		return startOfDay + Math.trunc(temporalHourMillis(startOfDay, endOfDay) * hours);
	}
	/**
	...
	*/
	getTemporalHour72(forceSeaLevel) {
		...
		return [alot72, (this.sunsetOffset(72, false, forceSeaLevel).getTime() - alot72.getTime()) / 12];
	}
	/**
	...
	*/
	getTemporalHourByDeg(angle) {
		...
		return [alot, (this.timeAtAngle(angle, false).getTime() - alot.getTime()) / 12];
	}
	/**
	...
	*/
	beinHaShmashos() {
		...
		if (isNaN(millis)) return tzeit;
		return /* @__PURE__ */ new Date(millis - 81e4);
	}
	/**
	...
	*/
	getMidnightLastNight() {
		return this.plainDate.toZonedDateTime({ timeZone: this.gloc.getTimeZone() });
	}
	/**
	* Used by Molad based _zmanim_ to determine if _zmanim_ occur during the current day.
	...
	*/
	getMidnightTonight() {
		return this.plainDate.add({ days: 1 }).toZonedDateTime({ timeZone: this.gloc.getTimeZone() });
	}
	/**
	...
	*/
	getMoladBasedTime(moladBasedTime, alos, tzais, techila) {
		...
		if (Temporal.ZonedDateTime.compare(moladBasedTime, lastMidnight) < 0 || Temporal.ZonedDateTime.compare(moladBasedTime, midnightTonight) > 0) return null;
		if (alos === null || tzais === null) return moladBasedTime.withTimeZone(this.gloc.getTimeZone());
		if (Temporal.ZonedDateTime.compare(moladBasedTime, alos) > 0 && Temporal.ZonedDateTime.compare(moladBasedTime, tzais) < 0) return techila ? tzais : alos;
	...
	*/
	static formatTime(dt, timeFormat) {
		if (isNaN(dt.getTime())) return "XX:XX";
		const time = timeFormat.format(dt);
		if (time.charCodeAt(0) === 50 && time.charCodeAt(1) === 52 && time.charCodeAt(2) === 58) return "00" + time.substring(2);
		return time;
	}
	/**
	...
	*/
	static roundTime(dt) {
		...
		if (isNaN(millis)) return dt;
		const millisOnly = dt.getMilliseconds();
		const seconds = dt.getSeconds();
		if (seconds === 0 && millisOnly === 0) return dt;
	...
	*/
	static formatISOWithTimeZone(tzid, date) {
		if (isNaN(date.getTime())) return "0000-00-00T00:00:00Z";
	...
	*/
	sunriseOffset(offset, roundMinute = true, forceSeaLevel = false) {
		...
		if (isNaN(sunrise.getTime())) return sunrise;
		if (roundMinute) {
			if (offset > 0 && sunrise.getSeconds() >= 30) offset++;
	...
	*/
	sunsetOffset(offset, roundMinute = true, forceSeaLevel = false) {
		...
		if (isNaN(sunset.getTime())) return sunset;
		if (roundMinute) {
			if (offset > 0 && sunset.getSeconds() >= 30) offset++;
	...
	*/
	static makeSunsetAwareHDate(gloc, date, useElevation) {
		const sunset = new Zmanim(gloc, date, useElevation).sunset();
		let hd = new HDate(date);
		const sunsetMillis = sunset.getTime();
		if (isNaN(sunsetMillis)) return hd;
		if (date.getTime() >= sunsetMillis) hd = hd.next();
		return hd;
	}
};
//#endregion
//#region node_modules/@hebcal/core/dist/esm/modern.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var SUN$1 = 0;
var TUE$1 = 2;
var FRI$2 = 5;
...
function dateYomHaShoah(year) {
	...
	if (nisan27dt.getDay() === FRI$2) nisan27dt = new HDate(26, NISAN$3, year);
	else if (nisan27dt.getDay() === SUN$1) nisan27dt = new HDate(28, NISAN$3, year);
	return nisan27dt;
}
/**
...
function dateYomHaZikaron(year) {
	...
	let day;
	const pdow = new HDate(15, NISAN$3, year).getDay();
	if (pdow === SUN$1) day = 2;
	else if (pdow === SAT$2) day = 3;
	else if (year < 5764) day = 4;
	else if (pdow === TUE$1) day = 5;
	else day = 4;
	return new HDate(day, IYYAR$1, year);
}
//#endregion
//#region node_modules/@hebcal/core/dist/esm/sedra.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var INCOMPLETE = 0;
var REGULAR = 1;
var COMPLETE = 2;
...
var Sedra = class {
	...
	*/
	constructor(hyear, il) {
		...
		this.il = Boolean(il);
		let key = `${leap}${rhDay}${yearType(hyear)}`;
		if (types[key]) {
		...
		if (!this.theSedraArray) throw new Error(`improper sedra year type ${key} calculated for ${hyear}`);
	}
	/**
	...
	*/
	find(parsha) {
		if (typeof parsha === "number") {
			if (parsha >= parshiot.length || parsha < 0 && !isValidDouble(parsha)) throw new RangeError(`Invalid parsha number: ${parsha}`);
			return this.findInternal(parsha);
		}
		if (typeof parsha === "string") {
			const num = parsha2id.get(parsha);
			if (typeof num === "number") return this.find(num);
			if (parsha.includes("-")) {
				if (parsha === CHMPESACH || parsha === CHMSUKOT) return this.findInternal(parsha);
		...
		if (Array.isArray(parsha)) {
			...
			if (plen !== 1 && plen !== 2 || typeof parsha[0] !== "string") throw new TypeError(`Invalid parsha argument: ${JSON.stringify(parsha)}`);
			if (plen === 1) return this.find(parsha[0]);
			const p1 = parsha[0];
			const p2 = parsha[1];
			const num1 = parsha2id.get(p1);
			const num2 = parsha2id.get(p2);
			if (typeof num1 !== "number" || typeof num2 !== "number" || num2 !== num1 + 1 || !isValidDouble(-num1)) throw new RangeError(`Unrecognized parsha name: ${p1}-${p2}`);
	...
	*/
	findContaining(parsha) {
		...
		if (hdate) return hdate;
		if (typeof parsha === "number") {
			const p1 = -parsha;
			if (isValidDouble(p1)) return this.find(p1);
			else return this.find(p1 + 1);
		} else {
			const num = parsha2id.get(parsha);
			if (num) {
				const p1 = -num;
	...
	*/
	lookup(hd) {
		...
		if (isNaN(abs)) throw new TypeError(`Bad date argument: ${hd}`);
		if (abs < this.rh) throw new RangeError(`Date ${hd} before start of Hebrew year ${this.year}`);
		...
		if (index === void 0) return getSedra(this.year + 1, this.il).lookup(saturday);
	...
	*/
	lookupWeekday(hd) {
		...
		if (isNaN(abs)) throw new TypeError(`Bad date argument: ${hd}`);
		if (abs < this.rh) throw new RangeError(`Date ${hd} before start of Hebrew year ${this.year}`);
		const day = new HDate(abs).getDay();
	...
	findWeekdayParsha(saturday) {
		...
		for (let sat2 = saturday.abs() + 7; sat2 <= endAbs; sat2 += 7) {
			const parsha2 = (sat2 > endOfYear ? getSedra(hyear + 1, il) : sedra).lookup(sat2);
...
var yearStartVayeilech = [
	51,
	52,
	CHMSUKOT
];
var yearStartHaazinu = [
	52,
	YK,
	CHMSUKOT
];
...
var r020 = range$1(0, 20);
var r027 = range$1(0, 27);
var r3340 = range$1(33, 40);
var r4349 = range$1(43, 49);
var r4350 = range$1(43, 50);
/**
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/staticHolidays.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/HolidayEvent.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
/**
...
var HolidayEvent = class extends Event {
	...
	*/
	...
	getEmoji() {
		...
		if (this.getFlags() & flags.SPECIAL_SHABBAT) return "🕍";
		return "✡️";
	}
	getCategories() {
		if (this.cholHaMoedDay) return [
			"holiday",
			"major",
			"cholhamoed"
		];
		...
		switch (this.getDesc()) {
			...
			case holidayDesc.ROSH_HASHANA_LABEHEMOT: return ["holiday", "minor"];
		}
	...
	*/
	render(locale) {
		return smartApostrophe(super.render(locale));
	}
	/**
	* Returns a brief (translated) description of this event.
	...
	*/
	renderBrief(locale) {
		return smartApostrophe(super.renderBrief(locale));
	}
};
/**
...
var AsaraBTevetEvent = class extends HolidayEvent {
	...
	urlDateSuffix() {
		return isoDateString(this.greg()).replaceAll("-", "");
	}
};
var chanukahEmoji = "🕎";
var KEYCAP_DIGITS = [
...
var RoshChodeshEvent = class extends HolidayEvent {
	...
	*/
	render(locale) {
		const monthName = this.getDesc().substring(13);
		const monthName1 = smartApostrophe(Locale.gettext(monthName, locale));
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/YomKippurKatanEvent.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
...
var YomKippurKatanEvent = class extends HolidayEvent {
	...
	*/
	render(locale) {
		const monthName = smartApostrophe(Locale.gettext(this.nextMonthName, locale));
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/holidays.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
/**
...
function getHolidaysOnDate(date, il) {
	...
	const events = getHolidaysForYear_(hd.getFullYear()).get(hdStr);
	if (il === void 0 || events === void 0) return events;
...
function getHolidaysForYear_(year) {
	if (typeof year !== "number") throw new TypeError(`bad Hebrew year: ${year}`);
	if (year < 1 || year > 32658) throw new RangeError(`Hebrew year ${year} out of range 1-32658`);
	...
	function add(...events) {
		for (const ev of events) {
			...
			if (typeof arr === "object") {
				if (arr[0].getFlags() & EREV$2) arr.unshift(ev);
				else arr.push(ev);
			} else map.set(key, [ev]);
		}
	}
	for (const h of staticHolidays) {
		...
		add(new HolidayEvent(hd, h.desc, h.flags, attrs));
	}
	add(new RoshHashanaEvent(RH, year, CHAG | LIGHT_CANDLES_TZEIS$2));
	add(new HolidayEvent(new HDate(RH.getDay() === THU ? 4 : 3, TISHREI$1, year), holidayDesc.TZOM_GEDALIAH, MINOR_FAST$1));
	add(new HolidayEvent(new HDate(HDate.dayOnOrBefore(SAT$1, 7 + RH.abs())), holidayDesc.SHABBAT_SHUVA, SPECIAL_SHABBAT$1));
	add(new HolidayEvent(HDate.shortKislev(year) ? new HDate(1, TEVET, year) : new HDate(30, KISLEV, year), holidayDesc.CHAG_HABANOT, MINOR_HOLIDAY$1));
	add(new ChanukahEvent(new HDate(24, KISLEV, year), holidayDesc.CHANUKAH_1_CANDLE, EREV$2 | MINOR_HOLIDAY$1 | CHANUKAH_CANDLES$1, void 0));
	for (let candles = 2; candles <= 8; candles++) add(new ChanukahEvent(new HDate(23 + candles, KISLEV, year), `Chanukah: ${candles} Candles`, MINOR_HOLIDAY$1 | CHANUKAH_CANDLES$1, candles - 1));
	...
	if (pesach.getDay() === SUN) add(new HolidayEvent(new HDate(16, ADAR_II, year), holidayDesc.PURIM_MESHULASH, MINOR_HOLIDAY$1));
	...
	if (nisan27dt) add(new HolidayEvent(nisan27dt, holidayDesc.YOM_HASHOAH, MODERN_HOLIDAY$1));
	const yomHaZikaronDt = dateYomHaZikaron(year);
	if (yomHaZikaronDt) add(new HolidayEvent(yomHaZikaronDt, holidayDesc.YOM_HAZIKARON, MODERN_HOLIDAY$1, emojiIsraelFlag), new HolidayEvent(yomHaZikaronDt.next(), holidayDesc.YOM_HAATZMA_UT, MODERN_HOLIDAY$1, emojiIsraelFlag));
	for (const h of staticModernHolidays) if (year >= h.firstYear) {
		let hd = new HDate(h.dd, h.mm, year);
		const dow = hd.getDay();
		if (h.friSatMovetoThu && (dow === FRI$1 || dow === SAT$1)) hd = hd.onOrBefore(THU);
		else if (h.friPostponeToSun && dow === FRI$1) hd = new HDate(hd.abs() + 2);
		else if (h.satPostponeToSun && dow === SAT$1) hd = hd.next();
		const mask = h.chul ? MODERN_HOLIDAY$1 : MODERN_HOLIDAY$1 | IL_ONLY$1;
		const ev = new HolidayEvent(hd, h.desc, mask);
		if (!h.suppressEmoji) ev.emoji = "🇮🇱";
		add(ev);
	}
	...
	add(new HolidayEvent(new HDate(HDate.dayOnOrBefore(SAT$1, av9abs)), holidayDesc.SHABBAT_CHAZON, SPECIAL_SHABBAT$1), new HolidayEvent(av9dt.prev(), holidayDesc.EREV_TISHA_BAV, EREV$2 | MAJOR_FAST$1, av9attrs), new HolidayEvent(av9dt, av9title, MAJOR_FAST$1, av9attrs), new HolidayEvent(new HDate(HDate.dayOnOrBefore(SAT$1, av9abs + 7)), holidayDesc.SHABBAT_NACHAMU, SPECIAL_SHABBAT$1));
	...
	for (let month = 1; month <= monthsInYear; month++) {
		...
		} else if (month !== TISHREI$1) add(new RoshChodeshEvent(new HDate(1, month, year), monthName));
	}
	for (let month = months.IYYAR; month <= monthsInYear; month++) {
		const nextMonth = month + 1;
		if (nextMonth === TISHREI$1 || nextMonth === months.CHESHVAN || nextMonth === TEVET) continue;
		let ykk = new HDate(29, month, year);
		const dow = ykk.getDay();
		if (dow === FRI$1 || dow === SAT$1) ykk = ykk.onOrBefore(THU);
		const nextMonthName = HDate.getMonthName(nextMonth, year);
		add(new YomKippurKatanEvent(ykk, nextMonthName));
	}
	for (const month of [CHESHVAN, IYYAR]) {
		const roshChodesh = new HDate(1, month, year);
		let shabbos = new HDate(HDate.dayOnOrBefore(SAT$1, roshChodesh.abs() + 6));
		if (shabbos.abs() === roshChodesh.abs()) shabbos = new HDate(shabbos.abs() + 7);
		const fastDays = [
			2,
			5,
			9
		].map((offset) => new HDate(shabbos.abs() + offset));
		if (month === IYYAR && fastDays[2].getDate() === 14) fastDays[2] = new HDate(17, IYYAR, year);
		for (const hd of fastDays) add(new HolidayEvent(hd, holidayDesc.TAANIT_BEHAB, MINOR_FAST$1 | BEHAB$1));
	}
	add(new HolidayEvent(getSedra(year, false).find(15), holidayDesc.SHABBAT_SHIRAH, SPECIAL_SHABBAT$1));
	const birkatHaChama = getBirkatHaChama(year);
	if (birkatHaChama) add(new HolidayEvent(new HDate(birkatHaChama), holidayDesc.BIRKAT_HACHAMAH, MINOR_HOLIDAY$1, { emoji: "☀️" }));
...
function getBirkatHaChama(year) {
	...
	for (let day = 0; day <= 40; day++) {
		...
		if ((abs + 1373429) % 10227 === 172) return abs;
	}
	return 0;
}
/**
...
months.AV;
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
flags.MAJOR_FAST | flags.MINOR_FAST;
flags.EREV;
//#endregion
//#region node_modules/@hebcal/core/dist/esm/TimedEvent.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var HAVDALAH = holidayDesc.HAVDALAH;
var CANDLE_LIGHTING = holidayDesc.CANDLE_LIGHTING;
/**
...
var TimedEvent = class extends Event {
	...
	*/
	constructor(date, desc, mask, eventTime, location, linkedEvent, options) {
		...
		if (linkedEvent !== void 0) this.linkedEvent = linkedEvent;
	}
	/**
	* @param [locale] Optional locale name (defaults to empty locale)
	*/
	...
	*/
	...
	getCategories() {
		switch (this.getDesc()) {
			case CANDLE_LIGHTING: return ["candles"];
			case HAVDALAH: return ["havdalah"];
			case holidayDesc.FAST_BEGINS:
			case holidayDesc.FAST_ENDS: return ["zmanim", "fast"];
			case holidayDesc.SOF_ZMAN_ACHILAT_CHAMETZ: return ["zmanim", "achilasChametz"];
			case holidayDesc.BIUR_CHAMETZ: return ["zmanim", "biurChametz"];
		}
		return ["unknown"];
	}
};
...
var HavdalahEvent = class extends TimedEvent {
	...
	constructor(date, mask, eventTime, location, havdalahMins, linkedEvent, options) {
		...
		if (havdalahMins) this.havdalahMins = havdalahMins;
	}
	/**
	* @param [locale] Optional locale name (defaults to empty locale)
	*/
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/candles.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
...
function makeCandleEvent(ev, hd, options, isFriday, isSaturday) {
	...
	if (ev !== void 0) {
		if (!isFriday) {
			if (mask & (LIGHT_CANDLES_TZEIS$1 | flags.CHANUKAH_CANDLES)) useHavdalahOffset = true;
	...
	const zmanim = new Zmanim(location, hd, Boolean(options.useElevation));
	const time = useHavdalahOffset && !offset ? zmanim.tzeit(options.havdalahDeg) : zmanim.sunsetOffset(offset, true);
	if (isNaN(time.getTime())) return;
	if (havdalahTitle) return new HavdalahEvent(hd, mask, time, location, options.havdalahMins, ev, options);
	else {
		mask |= LIGHT_CANDLES$1;
		return new CandleLightingEvent(hd, mask, time, location, ev, options);
	}
}
var FAST_BEGINS = holidayDesc.FAST_BEGINS;
var FAST_ENDS = holidayDesc.FAST_ENDS;
/**
...
function makeFastEndTime(zmanim, isTishaBav, options) {
	if (isTishaBav) return zmanim.tzeit(TZEIT_TUCAZINSKY);
	const fastEndMins = options.fastEndMins;
	if (typeof fastEndMins === "number" && fastEndMins !== 0) return zmanim.sunsetOffset(Math.abs(fastEndMins), true);
	const fastEndDeg = options.fastEndDeg;
	if (typeof fastEndDeg === "number" && fastEndDeg !== 0) return zmanim.tzeit(Math.abs(fastEndDeg));
	if (options.il) return zmanim.sunsetOffset(MINOR_FAST_END_MINUTES_IL, true);
...
function makeFastStartEnd(ev, options) {
	...
	if (desc === "Yom Kippur") throw new RangeError("YK does not require this function");
	const hd = ev.getDate();
	const dt = hd.greg();
	const location = options.location;
	const zmanim = new Zmanim(location, dt, Boolean(options.useElevation));
	let startEvent;
	let endEvent;
	if (desc === "Erev Tish'a B'Av") {
		const sunset = zmanim.sunset();
		if (!isNaN(sunset.getTime())) startEvent = makeTimedEvent(ev, sunset, FAST_BEGINS, options);
	} else if (desc.startsWith("Tish'a B'Av")) {
		const fastEnd = makeFastEndTime(zmanim, true, options);
		if (!isNaN(fastEnd.getTime())) endEvent = makeTimedEvent(ev, fastEnd, FAST_ENDS, options);
	} else {
		const dawn = zmanim.alotHaShachar();
		if (!isNaN(dawn.getTime())) startEvent = makeTimedEvent(ev, dawn, FAST_BEGINS, options);
...
function makeTimedEvent(ev, time, desc, options) {
	const location = options.location;
	return new TimedEvent(ev.getDate(), desc, ev.getFlags(), time, location, ev, options);
}
/**
...
function makeWeekdayChanukahCandleLighting(ev, options) {
	...
	const candleLightingTime = new Zmanim(location, hd.greg(), useElevation).beinHaShmashos();
	if (isNaN(candleLightingTime.getTime())) return null;
	return new TimedChanukahEvent(ev, candleLightingTime, location);
}
//#endregion
//#region node_modules/@hebcal/core/dist/esm/sefira.json.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var sefira = {
	"ps67lines": [
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/omer.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var sefirot = {
	en: {
		infix: "within ",
...
function checkDay(omerDay) {
	if (omerDay < 1 || omerDay > 49) throw new RangeError(`Invalid Omer day ${omerDay}`);
}
function getWeeks(omerDay) {
	return [Math.floor((omerDay - 1) / 7) + 1, omerDay % 7 || 7];
}
function omerTodayIsEn(omerDay) {
	const [weekNumber, daysWithinWeeks] = getWeeks(omerDay);
	let str = `Today is ${omerDay} ${omerDay === 1 ? "day" : "days"}`;
	if (weekNumber > 1 || omerDay === 7) {
		...
		if (!day7) str += ` and ${daysWithinWeeks} ${daysWithinWeeks === 1 ? "day" : "days"}`;
	}
...
function omerTodayIsHe(omerDay) {
	...
	let str = "הַיּוֹם ";
	if (omerDay === 11) str += "אַחַד " + asar;
	else if (omerDay === 12) str += "שְׁנֵים " + asar;
	else if (12 < omerDay && omerDay < 20) str += ones[one] + " עָשָׂר";
	else if (omerDay > 9) {
		str += ones[one];
		if (one) {
			str += " ";
	...
	if (omerDay > 2) {
		if (omerDay > 20 || omerDay === 10 || omerDay === 20) str += tens[ten];
		if (omerDay < 11) str += ones[one] + " יָמִים ";
		else str += " " + yom + " ";
	} else if (omerDay === 1) str += yomEchad + " ";
	else str += shneiYamim + " ";
	if (omerDay > 6) {
		str = str.trim();
		str += ", שֶׁהֵם ";
		...
		if (weeks > 2) str += ones[weeks] + " שָׁבוּעוֹת ";
		else if (weeks === 1) str += "שָׁבֽוּעַ " + ones[1] + " ";
		else str += shnei + " " + shavuot + " ";
		if (days) {
			if (days === 2 || days === 3) str += "וּ";
			else if (days === 5) str += "וַ";
			else str += "וְ";
			if (days > 2) str += ones[days] + " יָמִים ";
			else if (days === 1) str += yomEchad + " ";
			else str += shneiYamim + " ";
		}
	}
	str += "לָעֽוֹמֶר";
...
var lamnatzeach = sefira.ps67lines.flatMap((x) => x.split(/[ ־]/));
...
var OmerEvent = class extends Event {
	...
	*/
	sefira(lang = "en") {
		if (lang !== "he" && lang !== "translit") lang = "en";
	...
	*/
	render(locale) {
		...
		return (isHebrewLocale ? gematriya(omer) : Locale.ordinal(omer, locale)) + " " + Locale.gettext("day of the Omer", locale);
	}
	/**
	...
	*/
	getEmoji() {
		...
		if (omerDay <= 20) codePoint = 9312 + omerDay - 1;
		else if (omerDay <= 35) codePoint = 12881 + omerDay - 21;
		else codePoint = 12977 + omerDay - 36;
	...
	*/
	getWeeks() {
		return this.daysWithinWeeks === 7 ? this.weekNumber : this.weekNumber - 1;
	}
	/**
	...
	*/
	getTodayIs(locale) {
		...
		const str = Locale.isHebrewLocale(locale) ? omerTodayIsHe(this.omer) : omerTodayIsEn(this.omer);
		if (locale === "he-x-nonikud") return Locale.hebrewStripNikkud(str);
		return str;
	}
	url() {
		const year = this.getDate().getFullYear();
		if (year < 5e3 || year > 6759) return;
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/tachanun.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
...
function tachanun0(hdate, il, checkNext) {
	...
	const abs = hdate.abs();
	if (dates.none.includes(abs)) return NONE$1;
	...
	if (!dates.some.includes(abs)) ret.allCongs = true;
	if (dow !== 6) ret.shacharit = true;
	const tomorrow = abs + 1;
	if (checkNext && !dates.yesPrev.includes(tomorrow)) ret.mincha = tachanun0(new HDate(tomorrow), il, false).shacharit;
	else ret.mincha = dow !== 5;
	if (ret.allCongs && !ret.mincha && !ret.shacharit) return NONE$1;
	return ret;
}
function tachanunYear(year, il) {
	...
	const none = [new HDate(2, months.TISHREI, year)].concat(range(1, monthsInYear).map((month) => new HDate(1, month, year)), range(1, monthsInYear).filter((month) => HDate.daysInMonth(month, year) === 30).map((month) => new HDate(30, month, year)), range(1, HDate.daysInMonth(months.NISAN, year)).map((mday) => new HDate(mday, months.NISAN, year)), new HDate(18, months.IYYAR, year), range(1, 8 - (il ? 1 : 0)).map((mday) => new HDate(mday, months.SIVAN, year)), av9dt, new HDate(15, months.AV, year), new HDate(29, months.ELUL, year), range(9, 24 - (il ? 1 : 0)).map((mday) => new HDate(mday, months.TISHREI, year)), range(25, 33).map((mday) => new HDate(mday, months.KISLEV, year)), new HDate(15, months.SHVAT, year), new HDate(14, months.ADAR_II, year), shushPurim, leap ? new HDate(14, months.ADAR_I, year) : []);
	const some = [new HDate(14, months.IYYAR, year)].concat(range(1, 13).map((mday) => new HDate(mday, months.SIVAN, year)), range(20, 31).map((mday) => new HDate(mday, months.TISHREI, year)), year >= 5708 ? dateYomHaZikaron(year).next() : [], year >= 5727 ? new HDate(28, months.IYYAR, year) : []);
	const yesPrev = [
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/parshaName.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
/** @private */
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/ParshaEvent.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
/**
...
var ParshaEvent = class extends Event {
	...
	*/
	constructor(parsha) {
		if (arguments.length !== 1) throw new TypeError(`ParshaEvent constructor takes a single SedraResult argument; got ${arguments.length} arguments`);
		if (typeof parsha !== "object" || parsha === null || !Array.isArray(parsha.parsha) || parsha.parsha.length === 0 || parsha.parsha.length > 2 || !HDate.isHDate(parsha.hdate)) throw new TypeError(`Invalid SedraResult argument: ${JSON.stringify(parsha)}`);
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/parshaYear.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
//#endregion
//#region node_modules/@hebcal/core/dist/esm/getStartAndEnd.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var TISHREI = months.TISHREI;
/**
...
function getYear(options) {
	if (options.year !== void 0) return Number(options.year);
...
function getStartAndEnd(options) {
	...
	if (hasStart !== hasEnd) throw new TypeError("options.start requires options.end");
	if (hasStart && hasEnd) {
		const start = getAbs(options.start), end = getAbs(options.end);
		if (end - start > 365 * MAX_NUM_YEARS) throw new RangeError(`Date range exceeds ${MAX_NUM_YEARS} years`);
		return [start, end];
	}
	const isHebrewYear = Boolean(options.isHebrewYear);
	const theYear = getYear(options);
	if (isNaN(theYear)) throw new RangeError(`Invalid year ${options.year}`);
	if (isHebrewYear && theYear < 1) throw new RangeError(`Invalid Hebrew year ${theYear}`);
	const theMonth = getMonth(options);
	const numYears = Number(options.numYears) || 1;
	if (numYears > MAX_NUM_YEARS) throw new RangeError(`options.numYears exceeds ${MAX_NUM_YEARS}`);
	if (isHebrewYear) return startEndHebrew(theMonth, theYear, numYears);
	else return startEndGregorian(theMonth, theYear, numYears);
}
function getMonth(options) {
	if (options.month) {
		if (options.isHebrewYear) return HDate.monthNum(options.month);
		if (typeof options.month === "number") return options.month;
	}
	return NaN;
}
function startEndGregorian(theMonth, theYear, numYears) {
	...
	if (theYear < 100) startGreg.setFullYear(theYear);
	const startAbs = greg2abs(startGreg);
	let endAbs;
	if (theMonth) endAbs = startAbs + daysInGregMonth(theMonth, theYear) - 1;
	else {
		const endYear = theYear + numYears;
		const endGreg = new Date(endYear, 0, 1);
		if (endYear < 100) endGreg.setFullYear(endYear);
...
function startEndHebrew(theMonth, theYear, numYears) {
	...
	if (!theMonth && theYear > 1) startAbs--;
	return [startAbs, endAbs];
}
//#endregion
//#region node_modules/@hebcal/core/dist/esm/DailyLearning.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
...
var DailyLearning = class {
	...
	*/
	static addCalendar(name, calendar, startDate) {
		if (typeof calendar !== "function") throw new TypeError(`Invalid calendar function: ${calendar}`);
	...
	*/
	static lookup(name, hd, il) {
		...
		if (typeof cal === "object") return cal.fn(hd, il);
		return null;
	}
	/**
	...
	*/
	static getStartDate(name) {
		const cal = cals.get(name.toLowerCase());
		if (typeof cal === "object") return cal.startDate;
	}
	/**
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/MevarchimChodeshEvent.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
...
var MevarchimChodeshEvent = class extends Event {
	...
	*/
	constructor(date, monthName, memo, locale) {
		...
		if (memo) this.memo = memo;
			...
			const molad = new Molad(hyear, hmonth === HDate.monthsInYear(hyear) ? months.NISAN : hmonth + 1);
	...
	*/
	render(locale) {
		const monthName = smartApostrophe(Locale.gettext(this.monthName, locale));
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/calendar.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
/**
...
function calendar(options = {}) {
	...
	if (options.locale) {
		...
		if (locale && typeof locale !== "string") throw new TypeError(`Invalid options.locale: ${locale}`);
		if (!Locale.hasLocale(locale)) throw new TypeError(`Locale '${locale}' not found; did you forget to import @hebcal/locales?`);
	} else if (options.ashkenazi) options.locale = "ashkenazi";
	else options.locale = "en";
	const evts = [];
	let sedra;
	let holidaysYear;
	let beginOmer = -1;
	...
	const startGregYear = abs2greg(startAbs).getFullYear();
	...
	for (let abs = startAbs; abs <= endAbs; abs++) {
		...
		let candlesEv;
		const holidays = (holidaysYear.get(hd.toString()) || []).filter((ev) => ev.observedIn(il));
		for (const ev of holidays) candlesEv = appendHolidayAndRelated(candlesEv, evts, ev, options, isFriday, isSaturday, hasUserMask);
		const mm = hd.getMonth();
		const dd = hd.getDate();
		if (isFriday && options.candlelighting && mm === months.NISAN && dd === 13) {
			const biurEv = makeBiurChametzEvent(hd, options);
			if (biurEv) evts.push(biurEv);
		}
		if (options.sedrot && isSaturday) {
			const parsha0 = sedra.lookup(abs);
			if (!parsha0.chag) evts.push(new ParshaEvent(parsha0));
		}
		if (options.yizkor) {
		...
		if (typeof dailyLearning === "object" && dailyLearning !== null) {
			...
			if (numDailyLearning) evts.push(...events);
		}
		if (options.omer && abs >= beginOmer && abs <= endOmer) {
			const omerEv = makeOmerEvent(hd, abs - beginOmer + 1, options);
			evts.push(omerEv);
		}
		...
		if (!candlesEv && options.candlelighting && (isFriday || isSaturday)) {
			...
			if (isFriday && candlesEv && sedra) {
				...
				} else candlesEv.memo = Locale.gettext(parsha.parsha[0], options.locale);
			}
		}
		if (candlesEv instanceof HavdalahEvent && (options.havdalahMins === 0 || options.havdalahDeg === 0)) candlesEv = void 0;
		if (candlesEv) evts.push(candlesEv);
		if (options.addHebrewDates || options.addHebrewDatesForEvents && prevEventsLength !== evts.length - numDailyLearning) {
			...
			if (prevEventsLength === evts.length) evts.push(e2);
			else evts.splice(prevEventsLength, 0, e2);
		}
	}
	return evts;
}
...
function warnUnrecognizedOptions(options) {
	for (const k of Object.keys(options)) if (RECOGNIZED_OPTIONS[k] === void 0 && !unrecognizedAlreadyWarned.has(k)) {
		console.warn(`Ignoring unrecognized HebrewCalendar option: ${k}`);
		unrecognizedAlreadyWarned.add(k);
	}
	if (options.dailyLearning) for (const [k, val] of Object.entries(options.dailyLearning)) {
...
function checkCandleOptions(options) {
	...
	const location = options.location;
	if (location === void 0 || !(location instanceof Location)) throw new TypeError("options.candlelighting requires valid options.location");
	if (typeof options.havdalahMins === "number" && typeof options.havdalahDeg === "number") throw new TypeError("options.havdalahMins and options.havdalahDeg are mutually exclusive");
	if (typeof options.fastEndDeg === "number" && typeof options.fastEndMins === "number") throw new TypeError("options.fastEndDeg and options.fastEndMins are mutually exclusive");
	const min0 = options.candleLightingMins;
	let min = typeof min0 === "number" && !isNaN(min0) ? Math.trunc(min0) : 18;
	if (location.getIsrael() && Math.abs(min) === 18) min = overrideIsraelCandleMins(location);
	options.candleLightingMins = -1 * Math.abs(min);
	if (typeof options.havdalahMins === "number") options.havdalahMins = Math.trunc(Math.abs(options.havdalahMins));
	else if (typeof options.havdalahDeg === "number") options.havdalahDeg = Math.abs(options.havdalahDeg);
	else options.havdalahDeg = TZEIT_3SMALL_STARS;
	if (typeof options.fastEndDeg === "number") options.fastEndDeg = Math.abs(options.fastEndDeg);
	if (typeof options.fastEndMins === "number") options.fastEndMins = Math.trunc(Math.abs(options.fastEndMins));
}
...
function getMaskFromOptions(options) {
	if (typeof options.mask === "number") return setOptionsFromMask(options);
	const il = options.il || options.location?.getIsrael() || false;
	let mask = 0;
	if (!options.noHolidays) mask |= ROSH_CHODESH | YOM_TOV_ENDS | MINOR_FAST | SPECIAL_SHABBAT | MODERN_HOLIDAY | MAJOR_FAST | MINOR_HOLIDAY | EREV | CHOL_HAMOED | LIGHT_CANDLES | LIGHT_CANDLES_TZEIS | CHANUKAH_CANDLES;
	if (options.candlelighting) mask |= LIGHT_CANDLES | LIGHT_CANDLES_TZEIS | YOM_TOV_ENDS;
	if (options.noRoshChodesh) mask &= ~ROSH_CHODESH;
	if (options.noModern) mask &= ~MODERN_HOLIDAY;
	if (options.noMinorFast) mask &= ~MINOR_FAST;
	...
	if (options.sedrot) mask |= PARSHA_HASHAVUA;
	if (options.omer) mask |= OMER_COUNT;
	if (options.shabbatMevarchim) mask |= SHABBAT_MEVARCHIM;
	if (options.yomKippurKatan) mask |= YOM_KIPPUR_KATAN;
	if (options.behab) mask |= BEHAB;
	if (options.yizkor) mask |= YIZKOR;
	const dailyLearning = options.dailyLearning;
	if (typeof dailyLearning === "object" && dailyLearning !== null) {
		if (dailyLearning.dafYomi) mask |= DAF_YOMI;
		if (dailyLearning.mishnaYomi) mask |= MISHNA_YOMI;
		if (dailyLearning.nachYomi) mask |= NACH_YOMI;
		if (dailyLearning.yerushalmi) mask |= YERUSHALMI_YOMI;
	}
	return mask;
}
...
function setOptionsFromMask(options) {
	...
	if (m & (DAF_YOMI | MISHNA_YOMI | NACH_YOMI | YERUSHALMI_YOMI)) {
		...
		if (m & DAF_YOMI) options.dailyLearning.dafYomi = true;
		if (m & MISHNA_YOMI) options.dailyLearning.mishnaYomi = true;
		if (m & NACH_YOMI) options.dailyLearning.nachYomi = true;
		if (m & YERUSHALMI_YOMI) options.dailyLearning.yerushalmi = 1;
	}
...
function appendHolidayAndRelated(candlesEv, events, ev, options, isFriday, isSaturday, hasUserMask) {
	...
	if (!options.yomKippurKatan && eFlags & YOM_KIPPUR_KATAN || !options.behab && eFlags & BEHAB || options.noModern && eFlags & MODERN_HOLIDAY) return candlesEv;
	if (options.candlelighting && ev.getDesc() === holidayDesc.EREV_PESACH) {
		const evts = makeErevPesachChametzEvents(ev, options);
		if (evts.length) events.push(...evts);
	}
	...
	if (options.candlelighting && (isMajorFast || isMinorFast) && ev.getDesc() !== holidayDesc.YOM_KIPPUR) {
		...
		if (fastEv.startEvent && (isMajorFast || isMinorFast && !options.noMinorFast)) events.push(fastEv.startEvent);
	}
	if (eFlags & Number(options.mask) || !eFlags && !hasUserMask) {
		...
		if (!options.noHolidays || options.yomKippurKatan && eFlags & YOM_KIPPUR_KATAN || options.behab && eFlags & BEHAB) events.push(ev);
	}
	if ((isMajorFast || isMinorFast && !options.noMinorFast) && fastEv?.endEvent) events.push(fastEv.endEvent);
	return candlesEv;
}
function makeMoladAndMevarchimChodesh(hd, options) {
	...
	if (hmonth !== ELUL && hdate >= 23 && hdate <= 29) {
		...
		if (options.molad) evts.push(new MoladEvent(hd, hyear, monNext, options));
		if (options.shabbatMevarchim) {
			const nextMonthName = HDate.getMonthName(monNext, hyear);
			const memo = new Molad(hyear, monNext).render(options.locale, options);
...
function dailyLearningName(key, val) {
	if (key === "yerushalmi") return val === 2 ? "yerushalmi-schottenstein" : "yerushalmi-vilna";
	return key;
}
function makeDailyLearning(hd, dailyLearning, il) {
	const evts = [];
	for (const [key, val] of Object.entries(dailyLearning)) if (val) {
		const name = dailyLearningName(key, val);
		const learningEv = DailyLearning.lookup(name, hd, il);
		if (learningEv) evts.push(learningEv);
	}
	return evts;
}
function makeOmerEvent(hd, omerDay, options) {
	...
	if (options.candlelighting) {
		const location = options.location;
		const tzeit = new Zmanim(location, hd.prev(), false).tzeit(7.0833);
		if (!isNaN(tzeit.getTime())) omerEv.alarm = tzeit;
	}
	return omerEv;
}
function makeErevPesachChametzEvents(erevPesachEv, options) {
	...
	const zmanAchilas = new Zmanim(location, hd, useElevation).sofZmanTfilla();
	if (isNaN(zmanAchilas.getTime())) return [];
...
function makeBiurChametzEvent(hd, options) {
	const location = options.location;
	const time = new Zmanim(location, hd, Boolean(options.useElevation)).sofZmanBiurChametzGRA();
	if (isNaN(time.getTime())) return;
...
//#endregion
//#region node_modules/@hebcal/core/dist/esm/hallel.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
var NONE = 0;
var HALF = 1;
var WHOLE = 2;
...
function hallel_(events, hdate) {
	...
	for (const ev of events) {
		...
		if (desc.startsWith("Chanukah") || desc.startsWith("Shavuot") || desc.startsWith("Sukkot") || month === months.NISAN && (mday === 15 || mday === 16) && mask & flags.CHAG || desc === holidayDesc.YOM_HAATZMA_UT || desc === holidayDesc.YOM_YERUSHALAYIM) return WHOLE;
		if (mask & flags.ROSH_CHODESH || desc.startsWith("Pesach") && desc !== holidayDesc.PESACH_I && desc !== holidayDesc.PESACH_II) return HALF;
	}
	return NONE;
}
//#endregion
//#region node_modules/@hebcal/core/dist/esm/hebcal.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
/**
...
var HebrewCalendar = class {
	...
	*/
	static eruvTavshilin(date, il) {
		if (date.getDay() < 3 || date.getDay() > 4) return false;
...
function isChag(date, il) {
	return (getHolidaysOnDate(date, il) || []).filter((ev) => ev.getFlags() & flags.CHAG).length !== 0;
}
//#endregion
//#region node_modules/@hebcal/core/dist/esm/index.js
/*! @hebcal/core v6.9.2, distributed under GPLv2 https://www.gnu.org/licenses/gpl-2.0.txt */
//#endregion
export { HavdalahEvent as a, HDate as c, CandleLightingEvent as i, Locale as l, ParshaEvent as n, Location as o, OmerEvent as r, flags as s, HebrewCalendar as t };
